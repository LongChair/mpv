...
static int init(struct ra_hwdec *hw)
{
    ...
    MP_VERBOSE(hw,"Extensions : %s\n Glext : %s\n", exts, gl->extensions);
    if (!strstr(exts, "EXT_image_dma_buf_import") ||
        !strstr(exts, "EGL_KHR_image_base") ||
        !strstr(gl->extensions, "GL_OES_EGL_image"))
    {
        MP_ERR(hw, "EGL doesn't support the following extensions : EXT_image_dma_buf_import, EGL_KHR_image_base, GL_OES_EGL_image\n");
        return -1;
    }
...
static int mapper_map(struct ra_hwdec_mapper *mapper)
{
    ...
    for (int l = 0; l < desc->nb_layers; l++) {
        ...
        ADD_ATTRIB(EGL_HEIGHT, p->tex[l]->params.h+8);
