...
static bool crtc_setup_atomic(struct ra_ctx *ctx, drmModeModeInfo mode)
{
    ...
    if (request) {
        ...
        drm_object_set_property(request, atomic_ctx->osd_plane, "SRC_W",   mode.hdisplay << 16);
        drm_object_set_property(request, atomic_ctx->osd_plane, "SRC_H",   mode.vdisplay << 16);
